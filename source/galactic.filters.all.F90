...
!!}
  ...
  <galacticFilter name="galacticFilterAll">
   <description>Implements a logical AND combination of multiple \refClass{galacticFilterClass} objects, passing only nodes that satisfy every filter in the list, enabling construction of complex selection criteria by composing simpler filters.</description>
...
contains
  ...
  function allConstructorInternal(filters) result(self)
    !!{
    Internal constructor for the \refClass{galacticFilterAll} filter class.
    !!}
    implicit none
    type(galacticFilterAll)                        :: self
