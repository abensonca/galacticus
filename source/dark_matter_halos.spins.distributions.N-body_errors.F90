...
contains
  ...
  function nbodyErrorsConstructorParameters(parameters) result(self)
    ...
    <inputParameter>
      ...
      <description>The mass (in $M_\odot$) of a single N-body simulation particle, used together with the halo particle count to estimate measurement errors in the halo spin parameter arising from discrete particle sampling.</description>
    </inputParameter>
    <inputParameter>
  ...
  double precision function nbodyErrorsDistributionAveraged(self,node,massLimit)
    !!{
    Compute the spin distribution averaged over all halos more massive than the given \mono{massLimit}.
    !!}
    use :: Calculations_Resets, only : Calculations_Reset
