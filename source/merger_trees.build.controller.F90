...
module Merger_Tree_Build_Controllers
  ...
  <functionClass>
   ...
   <description>Class providing merger tree build controllers — logic objects that govern when and how the
    tree builder places new nodes during tree construction. A controller decides whether a given node
    should be continued (branched further), constrains the range of allowed ``times'' (overdensity values)
    at each node, forces node insertion at specified times, and supplies the branching probability object
    to use. Controllers enable constrained trees, main-branch-only trees, and other specialized
