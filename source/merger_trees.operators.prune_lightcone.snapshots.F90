...
!!    along with Galacticus.  If not, see <http://www.gnu.org/licenses/>.
  ...
  type, extends(mergerTreeOperatorClass) :: mergerTreeOperatorPruneLightconeSnapshots
   ...
   contains
     ...
     !!]
     final     ::                        pruneLightconeSnapshotsDestructor
     procedure :: operatePreEvolution => pruneLightconeSnapshotsOperatePreEvolution
     procedure :: processNode         => pruneLightconeSnapshotsProcessNode
     procedure :: timeIntersect       => pruneLightconeSnapshotsTimeIntersect
  ...
  interface mergerTreeOperatorPruneLightconeSnapshots
     ...
     module procedure pruneLightconeSnapshotsConstructorInternal
  end interface mergerTreeOperatorPruneLightconeSnapshots
...
contains
  ...
  function pruneLightconeSnapshotsConstructorParameters(parameters) result(self)
    ...
    implicit none
    type   (mergerTreeOperatorPruneLightconeSnapshots)                :: self
    ...
    !!]
    self=mergerTreeOperatorPruneLightconeSnapshots(geometryLightcone_,satelliteOrphanDistribution_,outputTimes_,bufferIsolatedHalos,splitTrees)
    !![
  ...
  function pruneLightconeSnapshotsConstructorInternal(geometryLightcone_,satelliteOrphanDistribution_,outputTimes_,bufferIsolatedHalos,splitTrees) result(self)
    ...
    call pruneLightconeSnapshotsValidate(self)
    return
  ...
  subroutine pruneLightconeSnapshotsDestructor(self)
    ...
    !!}
    implicit none
    type(mergerTreeOperatorPruneLightconeSnapshots), intent(inout) :: self
  ...
  subroutine pruneLightconeSnapshotsOperatePreEvolution(self,tree)
    ...
    use :: Galacticus_Nodes              , only : mergerTree                    , nodeComponentBasic      , nodeComponentSatellite, treeNodeLinkedList, &
         &                                        treeNode
    ...
    implicit none
    class           (mergerTreeOperatorPruneLightconeSnapshots), intent(inout), target :: self
    ...
    double precision                                                                   :: radiusBuffer           , timeIntersect     , &
         &                                                                                timeInLightconeLatest
    logical                                                                            :: treeIntersectsLightcone, nodeIsNewRoot
    ...
    do while (treeWalkerAll%next(node))
       ! Skip nodes which are not to be processed.
       if (.not.self%processNode(node)) cycle
       ! Find the time at which the node intersects the lightcone (if any).
       timeIntersect=self%timeIntersect(node)
       ! If intersection with lightcone was detected then this tree can not be pruned - return immediately.
       if (timeIntersect > 0.0d0) then
  ...
  double precision function pruneLightconeSnapshotsTimeIntersect(self,node) result(timeIntersect)
    ...
    if (nodeIntersectsLightcone) then
       basic         => node %basic()
       timeIntersect =  basic%time ()
    else
       timeIntersect =  0.0d0
    end if
    return
  end function pruneLightconeSnapshotsTimeIntersect
