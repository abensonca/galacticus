...
status = subprocess.run("cd ..; scripts/aux/validateParameters.pl parameters.xml", shell=True)
    ...
    for fileName in sorted(os.listdir(validationDir)):
        ...
        if m:
            ...
            status = subprocess.run(
                f"cd ../../..; scripts/aux/validateParameters.pl testSuite/parameters/validation/{fileName}",
                shell=True
            )
